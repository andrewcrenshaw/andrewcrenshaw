## Andrew Crenshaw

Senior technology leader building in agentic AI — memory architectures, multi-agent frameworks, and applied products that pair enterprise depth with hands-on delivery.

Most organizations experimenting with AI agents are running single-agent demos against toy problems. I design and ship multi-agent orchestration systems with specialized agents operating under structured human governance — and I have extended that infrastructure to the hardest underlying problem: **memory**.

---

### What I'm Building

**[Autogenous Synthesis Forge](https://andrewcrenshaw.dev/autogenous-synthesis/)** — An AI-first development OS. An autonomous orchestrator decomposes tickets, dispatches 12+ specialist agents against machine-verifiable acceptance criteria, enforces TDD, and auto-revises on failure. Persistent scratchpads survive context limits; 150+ automated test suites turn quality into binary pass/fail. ~80% reduction in review load.

**[Strata](https://andrewcrenshaw.dev/strata/)** — Autonomous job-search operating system. Eight specialized agents collaborate through governed PostgreSQL to discover, evaluate, match, and help apply to jobs. Three-layer dedup eliminates 95% of noise before LLM scoring. Two public PyPI libraries: [strata-harvest](https://github.com/andrewcrenshaw/strata-harvest) · [strata-match](https://github.com/andrewcrenshaw/strata-match).

**Alexandria / Living Memory** — Persistent memory architecture for agents: a metabolic knowledge graph where institutional knowledge compounds across sessions through assumption registers, decision-quality tracking, and drift detection.

---

### Background

- **Independent AI Product Developer** (Apr 2026–present) — Building agentic memory systems, multi-agent development framework, and applied AI products
- **Head of GTM Engineering, Macmillan Learning** (2013–2026) — Built GTM engineering from a team of one to 30+; owned the platform supporting ~$250M in annual revenue; CRM consolidation that enabled 5x revenue growth
- **Executive Director, Morgan Stanley** (2002–2013) — P&L ownership on a $20M Fixed Income Global Sales technology program; AML/KYC transformation; consolidated 600+ reports into a unified Sales Desktop

BS Physics · Behavioral Neuroscience · Fine & Applied Arts — University of Oregon (Magna Cum Laude)

---

### Links

[andrewcrenshaw.dev](https://andrewcrenshaw.dev) · [LinkedIn](https://www.linkedin.com/in/andrewcrenshaw/)

---

### Activity & Velocity

<!-- Auto-generated daily by .github/workflows/metrics.yml — aggregate stats only; no private code, commit messages, or repo names are exposed. -->
<picture>
  <img src="./github-metrics.svg" alt="GitHub activity metrics — commit heatmap, lines of code, and language breakdown across public and private work">
</picture>

